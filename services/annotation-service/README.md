...
- `POST /annotations/studies/{study_id}` -- create a draft annotation (role: `annotator`/`admin`). Payload is validated against the type's JSON Schema.
- `GET /annotations/studies/{study_id}?status=submitted` -- list annotations in a study, optionally filtered by status (e.g. a reviewer's queue)
