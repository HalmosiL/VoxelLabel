...
| Something answers "unavailable right now -- try again" (503) | A dependency is down. `curl http://localhost:<port>/health/ready` on each service (8001 ingestion, 8002 data, 8003 annotation, 8004 admin, 8010 viewer) names which one: `database`, `storage` (MinIO), `queue` (Redis) or a platform service. `/health` only says the process runs |
